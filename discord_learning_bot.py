...
class LearningEngine:
    ...
    def __init__(self, data_dir: str = "bot_data"):
        ...
        if self.model is None:
            ...
            # Adaptive model sizing based on vocabulary
            # Larger vocab = need more capacity
            if vocab_size < 500:
                # Small vocab: minimal model
                embedding_dim = 32
                hidden_dim = 64
            elif vocab_size < 2000:
                # Medium vocab: standard model
                embedding_dim = 48
                hidden_dim = 96
            elif vocab_size < 5000:
                # Large vocab: bigger model
                embedding_dim = 64
                hidden_dim = 128
            else:
                # Very large vocab: maximum capacity
                embedding_dim = 96
                hidden_dim = 192
            
            print(f"Initializing model: vocab={vocab_size}, embed={embedding_dim}, hidden={hidden_dim}")
    ...
    def _resize_model(self):
        """Resize model when vocabulary grows"""
        new_vocab_size = self.tokenizer.get_vocab_size()
        old_vocab_size = self.model.vocab_size
        
        if new_vocab_size > old_vocab_size:
            # Check if we need to upgrade model capacity
            old_hidden = self.model.hidden_dim
            old_embed = self.model.embedding_dim
            
            # Determine new dimensions based on vocab size
            if new_vocab_size < 500:
                new_embed, new_hidden = 32, 64
            elif new_vocab_size < 2000:
                new_embed, new_hidden = 48, 96
            elif new_vocab_size < 5000:
                new_embed, new_hidden = 64, 128
            else:
                new_embed, new_hidden = 96, 192
            
            # If dimensions changed, recreate model with new size
            if new_embed != old_embed or new_hidden != old_hidden:
                print(f"Upgrading model capacity: {old_embed}/{old_hidden} → {new_embed}/{new_hidden}")
                ...
                # Copy over old embeddings (pad if needed)
                min_embed = min(old_embed, new_embed)
                new_model.embeddings[:old_vocab_size, :min_embed] = old_embeddings[:, :min_embed]
                
                # Copy LSTM weights if dimensions match
                if old_hidden == new_hidden:
                    # Can copy LSTM weights directly
                    ...
                    # Keep new random LSTM weights (fresh start for new capacity)
                    print("  LSTM weights reset due to dimension change")
                
                # Replace model
                self.model = new_model
                return
            
            # Just expand embeddings and output (no dimension change)
            # Expand embeddings
