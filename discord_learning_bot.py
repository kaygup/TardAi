...
class TinyLSTM:
    ...
    def train_step(self, input_tokens: List[int], target_token: int) -> float:
        """
        Single training step with simplified backprop.
        ...
        # Simplified gradient descent on output layer only (for efficiency)
        # Full BPTT would be too slow for real-time learning
        d_logits = probs.copy()
        ...
        self.W_out -= self.lr * np.outer(h_last, d_logits)
        self.b_out -= self.lr * d_logits
        
        # Update embeddings (use up to last 10 tokens for better context learning)
        for step in cache[-10:]:  # Last 10 tokens for improved backprop
            token_id = step['token_id']
            d_embed = d_logits[:self.embedding_dim] if len(d_logits) >= self.embedding_dim else np.zeros(self.embedding_dim)
            self.embeddings[token_id] -= self.lr * 0.1 * d_embed[:self.embedding_dim]
...
class LearningEngine:
    ...
    def learn_from_message(self, message: str):
        ...
        if len(tokens) > 2:  # Need at least START, word, END
            ...
            for i in range(2, len(tokens)):
                ...
                if random.random() < 0.8:  # 80% training rate
    ...
    def generate_response(self, context: str = "", temperature: float = 1.0) -> str:
        ...
        adjusted_temp = temperature * max(0.5, 1.0 - (self.training_count / 1000))
        ...
        # Fallback for very early stages
        if not response or len(response) < 2:
            if self.training_count < 20:
                return random.choice(["...", "hmm", "?", "ok", "hello"])
            elif self.training_count < 50:
                # Echo random words from vocabulary
                common_words = [word for word, _ in self.tokenizer.word_freq.most_common(10)]
                if common_words:
                    return " ".join(random.sample(common_words, min(3, len(common_words))))
